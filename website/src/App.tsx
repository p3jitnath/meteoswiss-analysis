  ...
  return (
    <>
      ...
      <main>
        ...
        <figure className="panel timeline-panel" id="annual" data-reveal><div className="section-head"><figcaption><p className="figure-number">02 / Annual record</p><h2>Annual {season.summary.season_label} temperature anomalies</h2></figcaption><div className="selection" aria-live="polite"><b>{displayed?.year}</b><strong>{displayed && signed(displayed.anomaly_c)}</strong><small>{displayed?.complete ? `warm rank ${displayed.rank}` : `${displayed?.days_observed}/${displayed?.days_expected} days, provisional`}</small></div></div><YearScrubber records={season.records} selected={displayed} onSelect={setSelected} /><div className="timeline-scroll"><Timeline records={season.records} onSelect={setSelected} /></div><p className="timeline-axis-label">Year</p><p className="hint"><span className="hint-wide">Interactive: scrub through years, or hover, tap or focus a bar. The outlined final bar is a like-for-like season-to-date anomaly and is not an estimate of the final summer mean.</span><span className="hint-phone">Drag the year control, or tap a bar. The outlined final bar is provisional.</span></p></figure>
