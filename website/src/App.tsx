...
function SwitzerlandMap() {
  const outline = "M 569.4,171.6 L 565,181.9 566.4,188.7 559.2,201.7 570,209.3 568.2,222.8 550.4,220.7 538,208.8 525.5,212.3 518.8,223.5 518.1,237.1 531.8,243.9 526,254 534,265.9 529.1,270.8 520,272.4 510.6,251.2 505.1,248.8 481.3,252 475.8,261.3 459.3,260.2 448.2,248.2 447.1,228.9 441.1,233.9 434.6,228.4 426.5,231 423.5,238.6 427.9,255.1 421.9,270.7 405.8,285.3 402,297.3 395.2,300.8 396.8,308 392.6,312.5 402.5,322.8 395.2,335 382.9,334.1 384.4,325.2 379.4,315.2 367.1,309.6 375.1,296.5 372,292.9 347.2,287.7 327.5,270.1 328.4,242.4 322.8,236.2 312.9,239.1 308.7,249 285.4,264.4 283.7,267.2 290.5,276.3 288.2,287.1 278,292.7 273.2,307 262.7,309.8 254.7,320.3 219.9,309.4 198.4,321.4 163,324.1 144.8,299.3 137.1,299.2 139.3,289.4 127.9,285.8 134.3,264.8 125,252.9 129.4,242.1 108.6,235.6 71.2,245.2 60.7,257.6 68.7,269.4 47.9,285.2 30,286.5 32.9,280.2 30,273.8 45.7,268.4 51.3,250.7 41.5,241.3 42.7,233.2 52.5,220.6 49.2,215.6 86.5,187.8 88.1,159.1 114.8,147.1 117.7,138.2 149.7,112 150,104.9 159.3,98.8 155.4,92.8 139,95 153.9,79.6 151.8,73.8 176.7,74 175.2,81 183.5,85.2 203.7,81.8 204.7,73.7 213.1,73.2 215.6,67 212.9,65.5 231.4,57.3 234.1,57 232.5,60.9 228.1,62 237,65.2 253.3,57.2 262.7,58.4 264.4,62.7 280,62.7 302.9,53.1 317.4,59.5 337.6,53.1 342.2,58.1 347.8,47.7 329.9,50.2 321.9,46.2 330.6,30.8 341.9,25 347.1,26 349,31.9 352.2,26.6 360.5,31.8 361,41.7 370.6,37.7 380.7,47.7 396.9,44.1 427.7,48.6 460.6,66.7 473,79.6 471.7,88.7 453.4,117.5 456.4,133.5 452.2,140.4 497.9,148 499.9,161.8 528.3,174.3 539.1,171.3 543.1,162.4 550.4,162.5 560.4,151.1 569.9,160.3 Z";
  const cities = [{ name: "Basel", x: 225.6, y: 62.8, elbowX: 211, elbowY: 45, lineX: 174, labelX: 168, anchor: "end" }, { name: "Bern", x: 208.6, y: 158.5, elbowX: 190, elbowY: 145, lineX: 153, labelX: 147, anchor: "end" }, { name: "Geneva", x: 52.2, y: 274.9, elbowX: 70, elbowY: 292, lineX: 101, labelX: 107, anchor: "start" }, { name: "Zürich", x: 339.9, y: 91.4, elbowX: 358, elbowY: 74, lineX: 394, labelX: 400, anchor: "start" }] as const;
  return <figure className="abstract-map"><div className="map-country-label"><span className="identity-flag" aria-hidden="true" />Switzerland</div><svg viewBox="0 0 600 360" role="img" aria-labelledby="map-title map-description"><title id="map-title">Swiss temperature station locations</title><desc id="map-description">Outline map of Switzerland marking Basel, Bern, Geneva and Zürich.</desc><path className="country-outline" d={outline} />{cities.map((city) => <g className="map-city" key={city.name}><circle cx={city.x} cy={city.y} r="6" /><polyline points={`${city.x},${city.y} ${city.elbowX},${city.elbowY} ${city.lineX},${city.elbowY}`} /><text x={city.labelX} y={city.elbowY} textAnchor={city.anchor}>{city.name}</text></g>)}</svg><figcaption>Four-station composite</figcaption></figure>;
}
  ...
  return (
    <>
      ...
      <main>
        ...
        <figure className="panel timeline-panel" id="annual" data-reveal><div className="section-head"><figcaption><p className="figure-number">02 / Annual record</p><h2>Annual {season.summary.season_label} temperature anomalies</h2></figcaption><div className="selection" aria-live="polite"><b>{displayed?.year}</b><strong>{displayed && signed(displayed.anomaly_c)}</strong><small>{displayed?.complete ? `warm rank ${displayed.rank}` : `${displayed?.days_observed}/${displayed?.days_expected} days, provisional`}</small></div></div><YearScrubber records={season.records} selected={displayed} onSelect={setSelected} /><div className="timeline-scroll"><Timeline records={season.records} onSelect={setSelected} /></div><p className="timeline-axis-label">Year</p><p className="hint">Interactive: scrub through years, or hover, tap or focus a bar. The outlined final bar is a like-for-like season-to-date anomaly and is not an estimate of the final summer mean.</p></figure>
      ...
      <footer><span className="footer-meta"><span>Data retrieved {season.summary.as_of}</span><span>Source: MeteoSwiss</span><span className="license-note">Code licensed under MIT</span></span><span className="footer-links"><a href="https://github.com/p3jitnath/meteoswiss-analysis/blob/main/LICENSE" target="_blank" rel="noreferrer">MIT licence</a><a className="footer-code" href="https://github.com/p3jitnath/meteoswiss-analysis" target="_blank" rel="noreferrer"><GitHubMark /> Source code</a></span></footer>
    </>
  );
}
