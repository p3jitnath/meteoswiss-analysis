...
function SwitzerlandMap() {
  ...
  const cities = [{ name: "Basel", x: 225.6, y: 62.8, dx: -8, dy: -14, anchor: "end" }, { name: "Bern", x: 208.6, y: 158.5, dx: -10, dy: 5, anchor: "end" }, { name: "Geneva", x: 52.2, y: 274.9, dx: 12, dy: 18, anchor: "start" }, { name: "Zürich", x: 339.9, y: 91.4, dx: 12, dy: -10, anchor: "start" }] as const;
  return <figure className="abstract-map"><svg viewBox="0 0 600 360" role="img" aria-labelledby="map-title map-description"><title id="map-title">Swiss temperature station locations</title><desc id="map-description">Outline map of Switzerland marking Basel, Bern, Geneva and Zürich.</desc><path className="country-outline" d={outline} />{cities.map((city) => <g className="map-city" key={city.name}><circle cx={city.x} cy={city.y} r="6" /><line x1={city.x} y1={city.y} x2={city.x + city.dx * .65} y2={city.y + city.dy * .65} /><text x={city.x + city.dx} y={city.y + city.dy} textAnchor={city.anchor}>{city.name}</text></g>)}</svg><figcaption>Four-station composite</figcaption></figure>;
}
  ...
  return (
    <>
      ...
      <main>
        ...
        <figure className="panel timeline-panel" id="annual" data-reveal><div className="section-head"><figcaption><p className="figure-number">02 / Annual record</p><h2>Annual {season.summary.season_label} temperature anomalies</h2></figcaption><div className="selection" aria-live="polite"><b>{displayed?.year}</b><strong>{displayed && signed(displayed.anomaly_c)}</strong><small>{displayed?.complete ? `warm rank ${displayed.rank}` : `${displayed?.days_observed}/${displayed?.days_expected} days, provisional`}</small></div></div><YearScrubber records={season.records} selected={displayed} onSelect={setSelected} /><div className="timeline-scroll"><Timeline records={season.records} onSelect={setSelected} /></div><p className="hint">Interactive: scrub through years, or hover, tap or focus a bar. The outlined final bar is a like-for-like season-to-date anomaly and is not an estimate of the final summer mean.</p></figure>
